...
void check_for_deadlocks(void) { 
    ...
    if ((aktueller_status_a == hole_klausuren && aktueller_status_b == hole_liste) ||
		...
		printf("Verklemmung erkannt. Auflösung der Verklemmung wird durchgeführt.\n");
		// Jede Verklemmung ein Mitarbeiter wählen 
		static int i = 0;
		if(i % 2 == 0) {
		// Zwingt Mitarbeiter A, seine Betriebsmittel freizugeben
        if (aktueller_status_a == hole_klausuren)
            sem_post(&klausuren);
        else if (aktueller_status_a == hole_liste)
            sem_post(&liste);
		printf("Mitarbeiter A hat ein Betriebsmittel freigegeben.\n");
		} else {
        // Zwingt Mitarbeiter B, seine Betriebsmittel freizugeben
        if (aktueller_status_b == hole_klausuren)
            sem_post(&klausuren);
        else if (aktueller_status_b == hole_liste)
            sem_post(&liste);
        printf("Mitarbeiter B hat ein Betriebsmittel freigegeben.\n");
		}
		i++;
		}
	/* Ende des zu modifizierenden Bereichs in dieser Funktion */
